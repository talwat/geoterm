...
static uint8_t read_u8(int fd) { uint8_t v; read(fd, &v, 1); return v; }
static uint32_t read_u32(int fd) { uint32_t v; read(fd, &v, 4); return ntohl(v); }
static float read_f32(int fd) { float f; read(fd, &f, 4); return f; }
...
static void deserialize_player(int fd, Player *p) {
    ...
    if (p->has_guess) deserialize_coordinate(fd, &p->guess);
    else { uint8_t pad[2]; read(fd, pad, 2); }
}
